...
-   **Coming Soon Mode**: Admin-controlled gate at `/admin/coming-soon`. When enabled, non-authorised visitors see a branded `/coming-soon` page with a waitlist form (name, email, phone, message). Existing users (registered before mode was enabled), admin users, and emails on the tester whitelist bypass the gate. Admin can toggle the mode, manage the tester whitelist (add/remove emails with notes), and view/export all waitlist signups as CSV. Gate is enforced via `GET /api/coming-soon/access` checked at app root in `AppContent`. New DB tables: `waitlist`, `tester_whitelist`; new columns on `site_settings`: `coming_soon_mode`, `coming_soon_enabled_at`.
