...
export async function registerRoutes(
  ...
  app.post("/api/admin/cleanup-users", async (req, res) => {
    ...
    const safe = async (label: string, sql: string, params: any[]) => {
    ...
    try {
      ...
      await safe("chat_logs", `DELETE FROM chat_logs WHERE owner_id != $1 OR guest_id != $1`, [ADMIN_ID]);
      await safe("call_logs", `DELETE FROM call_logs WHERE owner_id != $1 OR guest_id != $1`, [ADMIN_ID]);
      await safe("search_history", `DELETE FROM search_history WHERE user_id != $1`, [ADMIN_ID]);
      await safe("user_preferences", `DELETE FROM user_preferences WHERE user_id != $1`, [ADMIN_ID]);
      await safe("wishlists", `DELETE FROM wishlists WHERE user_id != $1`, [ADMIN_ID]);
      await safe("reviews", `DELETE FROM reviews WHERE guest_id != $1`, [ADMIN_ID]);
      await safe("availability_overrides", `DELETE FROM availability_overrides WHERE created_by != $1`, [ADMIN_ID]);
      await safe("meal_plan_price_overrides", `DELETE FROM meal_plan_price_overrides WHERE created_by != $1`, [ADMIN_ID]);
      await safe("room_price_overrides", `DELETE FROM room_price_overrides WHERE created_by != $1`, [ADMIN_ID]);
      await safe("bookings", `DELETE FROM bookings WHERE guest_id != $1`, [ADMIN_ID]);
      await safe("kyc_applications", `DELETE FROM kyc_applications WHERE user_id != $1`, [ADMIN_ID]);
      await safe("property_deactivation_requests", `DELETE FROM property_deactivation_requests WHERE owner_id != $1`, [ADMIN_ID]);
      await safe("owner_referrals", `DELETE FROM owner_referrals WHERE referee_id != $1 OR referrer_id != $1`, [ADMIN_ID]);
      await safe("subscription_payments", `DELETE FROM subscription_payments WHERE owner_id != $1`, [ADMIN_ID]);
      await safe("invoices", `DELETE FROM invoices WHERE owner_id != $1`, [ADMIN_ID]);
      await safe("owner_subscriptions", `DELETE FROM owner_subscriptions WHERE owner_id != $1`, [ADMIN_ID]);
      await safe("conversations", `DELETE FROM conversations WHERE guest_id != $1 OR owner_id != $1`, [ADMIN_ID]);
      await safe("property_images", `DELETE FROM property_images WHERE property_id IN (SELECT id FROM properties WHERE owner_id != $1)`, [ADMIN_ID]);
      await safe("property_amenities", `DELETE FROM property_amenities WHERE property_id IN (SELECT id FROM properties WHERE owner_id != $1)`, [ADMIN_ID]);
      await safe("property_meal_plans", `DELETE FROM property_meal_plans WHERE property_id IN (SELECT id FROM properties WHERE owner_id != $1)`, [ADMIN_ID]);
